...
## Enemy Mantis - High damage with predictive movement

const HP: float = 50.0
const ATK: float = 24.0  # was 28
const SPEED: float = 180.0
const ATK_RANGE: float = 85.0
const ATK_COOL: float = 2.2
const CHARGE_TIME: float = 1.0
const WARN_RANGE: float = 120.0
# AI 智能参数
...
func _apply_appearance_variants() -> void:
	# 随机颜色变体 - 螳螂偏绿色系
	var base_color = Color(0.3, 0.45, 0.3, 1)
	...
	if head:
		head.color = _adjust_color(base_color, -0.02, 0.08, -0.02)
...
func _add_spots() -> void:
	...
	for i in range(spot_count):
		...
		spot.color = Color(0.18, 0.28, 0.18, 0.6)
...
func _add_stripes() -> void:
	...
	for i in range(stripe_count):
		...
		stripe.color = Color(0.18, 0.28, 0.18, 0.5)
...
func _physics_process(delta: float) -> void:
	...
	# Calculate predicted player position for tracking
	...
	if _is_in_caution_mode():
		prediction_time *= 1.5
	predicted_position = player.global_position + player_velocity * prediction_time

	var d = global_position.distance_to(predicted_position)
	...
	if d < atk_dist:
		...
		if d < WARN_RANGE:
			# Player detected - predictive chase
			is_patrolling = false
			move_timer -= delta
			if move_timer <= 0:
				move_timer = randf_range(0.5, 1.2)

				# 协作包围：螳螂利用速度优势绕后
				var surround_pos = _calculate_surround_position()
				var target_pos = predicted_position
			...
			global_position += dir * SPEED * delta * 0.35 * speed_mult
	...
	attack_direction = global_position.angle_to(predicted_position) - PI / 2
...
func _calculate_surround_position() -> Vector2:
	...
	# 螳螂速度块，包围时更激进
	return player.global_position + Vector2(cos(surround_angle), sin(surround_angle)) * 50.0
...
func perform_attack() -> void:
	...
	# Attack at predicted position
	var player = get_tree().get_first_node_in_group("player")
	if player and is_target_in_attack_indicator(player.global_position, ATK_RANGE + 24, PI / 2.5):
...
func die() -> void:
	...
	var main = get_tree().get_first_node_in_group("main")
	if main:
		main.spawn_resource(global_position, "mantis_remains")
	queue_free()
